...
| Both "Why keep votes private?" grids take input                                                                                                                                      | They're the real grid on a sample Session, so the left one moves and clears a Selection and the right one opens popovers. Nothing is sent anywhere                                                  |
...
| Desktop nav reads "Why keep votes private?", not the artboard's "Why hidden?"                                                                                                        | §7 item 1 names it that way, and it fits at 1200px                                                                                                                                                  |
...
| **"Why keep votes private?" removed** — the section, its header and menu links, and `EstimationGrid`'s `squareFit` | Cut from the page at your request. `squareFit` existed only so the two half-width comparison grids fit, so it went with them. §7 renumbers: Why I made this is now item 6, the footer item 9 |
