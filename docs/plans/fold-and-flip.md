...
- **Popover** — opens on click, _pins_ open, lists every name with no "+N more",
...
_Done when:_ a dev fixture screen under `src/routes/dev/` renders the full
...
- Rename to **Fold and Flip**. It is the display name; "Product Poker", "Jira
  ...
  | `index.html` `<head>`                                    | _no description or og tags at all_ | Added, not edited                                               |
  ...
  **Do not rename** `KEY_PREFIX` in
  ...
  infrastructure, and the Welcome repo cards (§7 item 6) name them literally.
...
_Done when:_ the logo cycles correctly, holds still under reduced motion, and no
...
_Done when:_ each primitive renders in the dev fixture at all three sizes, and
...
_Done when:_ an 11×11 Numerical grid is usable at 390px, the grid is fully
...
| Welcome's header variant waits for Stage 9                                                               | Its taller header with nav and a menu (§5, §7), and the footer (§7 item 9), have no other consumer. `PageLayout` gets them then                                                                                                                                                                                                                                                                                  |
...
_Done when:_ the wave plays once on entry, a chip previews and pins its
...
| Both "Why keep estimates private?" grids take input                                                                                                                                  | They're the real grid on a sample Session, so the left one moves and clears a Selection and the right one opens popovers. Nothing is sent anywhere                                                  |
...
| Desktop nav reads "Why keep estimates private?", not the artboard's "Why hidden?"                                                                                                    | §7 item 1 names it that way, and it fits at 1200px                                                                                                                                                  |
...
The Reveal notice's headline wording is whatever the real Reveal uses, so the
hero shows it too (it now reads "Estimating is closed").
...
| **"Why keep estimates private?" removed** — the section, its header and menu links, and `EstimationGrid`'s `squareFit` | Cut from the page at your request. `squareFit` existed only so the two half-width comparison grids fit, so it went with them. §7 renumbers: Why I made this is now item 6, the footer item 9               |
