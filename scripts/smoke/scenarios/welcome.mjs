...
export async function welcome({ browser, reporter }) {
  ...
  const desktop = await openPage(
    await browser.newContext({
      viewport: { width: SIZES.desktop, height: 900 },
    }),
  );
  ...
  check(
    ...
    await desktop
      .getByRole("heading", { name: "The Reveal", level: 2 })
      .isVisible(),
  );
  ...
  check(
    "Arriving pinned does not scroll the page",
    (await desktop.evaluate(() => window.scrollY)) === 0,
  );
  check(
    "Demo popover waits while the grid is off screen",
  ...
  await desktop.evaluate(() =>
    document
      .querySelector("main [data-square]")
      .closest(".grid")
  ...
  check(
    ...
    JSON.stringify(await popover.locator("li").allInnerTexts()) ===
      JSON.stringify(DEMO_CROWD),
  );
  ...
  const mobile = await openPage(
    await browser.newContext({
      viewport: { width: SIZES.mobile, height: 844 },
    }),
  );
  ...
  check(
    "Desktop nav hidden on a phone",
    !(await mobile.getByRole("navigation", { name: "Main" }).isVisible()),
  );
  ...
  check(
    "Button reads Close, expanded",
    (await closeButton.getAttribute("aria-expanded")) === "true",
  );

  await menu.getByRole("link", { name: "Why I made this" }).click();
  check("A row closes the menu", (await menu.count()) === 0);
  check(
    "A row follows its link",
    await landsOn(mobile, `${APP}/welcome#why-i-made-this`),
  );
