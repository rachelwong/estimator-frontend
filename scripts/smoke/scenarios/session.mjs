...
export async function session({ browser, reporter }) {
  ...
  check(
    "Admin sees End session",
    await admin
      .getByRole("button", { name: "End session & reveal" })
      .isVisible(),
  );
  check(
    "Admin sees share link",
    await admin.getByLabel("Session link").isVisible(),
  );

  const shareUrl = await admin.getByLabel("Session link").inputValue();
  check(
    "Share link points at /join",
    shareUrl === `${APP}/${sessionId}/join`,
    shareUrl,
  );
  ...
  check(
    "Grid is one Tab stop",
    (await admin.locator('main [data-square][tabindex="0"]').count()) === 1,
  );
  ...
  check(
    "Participant has no End session",
    (await fay
      .getByRole("button", { name: "End session & reveal" })
      .count()) === 0,
  );
  check(
    "Participant has no share link",
    (await fay.getByLabel("Session link").count()) === 0,
  );
  ...
  for (const [label, page] of [
    ["Admin", admin],
    ["Admin tab 2", adminTab2],
    ["Participant", fay],
  ]) {
    check(`${label} moved to /ended`, await landsOn(page, /\/ended$/));
  }
  check(
    "No error banner after ending",
    (await admin.getByRole("alert").count()) === 0,
  );
  await fay.getByRole("heading", { name: "The Reveal" }).waitFor();
  check(
    "The Reveal is the one h1",
    (await pageHeadings(fay)).join() === "The Reveal",
  );
  check(
    "Chip row label sits under it",
    await fay
      .getByRole("heading", { name: "your estimates", level: 2 })
      .isVisible(),
  );
  check(
    "Live Participant sees own name in Reveal",
    (await squareAriaLabel(fay, 4, 1)).includes("Fay"),
  );

  // "Your Square": the tab that watched the Session end still knows its
  // Selection. The Admin cleared theirs, so has none to mark.
  check(
    "Your Square is marked for Fay",
    (await squareAriaLabel(fay, 4, 1)) ===
      "Time 4, resources 1, Fay, your Selection",
  );
  check(
    "Your Square wears the ring",
    (await squareClasses(fay, 4, 1)).includes("outline-selection"),
  );
  check(
    "Only one Square is yours",
    (await fay
      .locator('main [data-square][class*="outline-selection"]')
      .count()) === 1,
  );
  check(
    "Admin with no Selection has no Square",
    (await admin
      .locator('main [data-square][class*="outline-selection"]')
      .count()) === 0,
  );
  check(
    "Participant chip on the Reveal",
    await fay.getByText("Participant", { exact: true }).isVisible(),
  );
  check(
    "Admin chip on the Reveal",
  ...
  fresh.on(
    "websocket",
    (ws) => ws.url().includes(SOCKET_HOST) && socketTraffic.push(ws.url()),
  );
  fresh.on(
    "request",
    (request) =>
      request.url().includes("/socket.io") && socketTraffic.push(request.url()),
  );
  ...
  check(
    "Fresh visitor redirected to /ended",
    fresh.url().endsWith(`/${sessionId}/ended`),
  );
  check(
    "No name prompt",
    (await fresh.locator('input[name="name"]').count()) === 0,
  );
  check(
    "No socket opened",
    socketTraffic.length === 0,
    socketTraffic.join(", "),
  );
  check(
    "Window reads revealed",
    await fresh.getByText("revealed", { exact: true }).isVisible(),
  );
  check(
    "Notice says Estimating is closed",
    await fresh.getByText("Estimating is closed").isVisible(),
  );
  check(
    "A fresh visitor has no role chip",
    (await fresh.getByText(/^(Admin|Participant)$/).count()) === 0,
  );
  check(
    "A fresh visitor has no Square",
    !(await squareAriaLabel(fresh, 4, 1)).includes("your Selection"),
  );
  ...
  check(
    "Crowded Square counts the people",
    (await cellText(fresh, 2, 3)) === "×4",
  );
  check("Fay in her Square", (await cellText(fresh, 4, 1)) === "Fa");
  check(
    "Crowded Square names everyone to a screen reader",
    (await squareAriaLabel(fresh, 2, 3)) ===
      "Time 2, resources 3, Bob, Cat, Dan, Eli",
  );
  check(
    "Empty Square says nobody",
    (await squareAriaLabel(fresh, 0, 0)) === "Time 0, resources 0, nobody",
  );
  check(
    "Cleared Squares are empty",
    (await cellText(fresh, 0, 0)) + (await cellText(fresh, 1, 0)) === "",
  );
  check(
    "Only chosen Squares are revealed",
    sameSquares(await revealed(fresh), [
      [2, 3],
      [4, 1],
    ]),
  );
  check(
    "No Square is pressed on the Reveal",
    (await chosen(fresh)).length === 0,
  );

  // The crowd ramp: headcount, not who.
  const fill = async (time, resource) =>
    (await squareClasses(fresh, time, resource))
      .split(" ")
      .find((c) => c.startsWith("bg-"));
  const fills = [await fill(0, 0), await fill(4, 1), await fill(2, 3)];
  check(
    "Fill follows headcount",
    fills.join() === "bg-crowd-0,bg-crowd-1,bg-crowd-4",
    fills.join(),
  );
  ...
  check(
    "Hover tooltip counts and points at the popover",
    tooltip === "T2 · R3 · 4 people · click for names",
    tooltip,
  );

  const crowd = await openSquarePopover(fresh, 2, 3);
  check(
    "Popover lists all four",
    crowd.sort().join() === "Bob,Cat,Dan,Eli",
    crowd.join(),
  );
  ...
  check(
    "An empty Square opens nothing",
    (await fresh.getByRole("dialog").count()) === 0,
  );
  ...
  const landed = await fresh
    ...
    .allInnerTexts();
  check(
    "Every picker has a chip",
    landed.sort().join() === "Bob,Cat,Dan,Eli,Fay",
    landed.join(),
  );

  // Ada cleared hers, Zed never picked.
  const abstained = await chips
    .filter({ hasText: "Abstained" })
    .allInnerTexts();
  const abstainedNames = abstained
    .map((text) => text.replace("Abstained", "").trim())
    .sort();
  check(
    "Ada (cleared) and Zed (never picked) are Abstained",
    abstainedNames.join() === "Ada,Zed",
    abstainedNames.join(),
  );
  ...
  check(
    "Hovering a chip previews its Square",
    chipTooltip === "T2 · R3 · 4 people · click for names",
    chipTooltip,
  );
  check(
    "The previewed Square lifts",
    (await squareClasses(fresh, 2, 3)).includes("shadow-px"),
  );

  const fayChip = fresh.getByRole("button", { name: "Fay", exact: true });
  await fayChip.click();
  const pinnedNames = await fresh
    .getByRole("dialog")
    .locator("li")
    .allInnerTexts();
  check(
    "Clicking a chip pins its popover",
    pinnedNames.join() === "Fay",
    pinnedNames.join(),
  );
  check(
    "The chip says it is expanded",
    (await fayChip.getAttribute("aria-expanded")) === "true",
  );
  await fayChip.click();
  await fresh.getByRole("dialog").waitFor({ state: "detached" });
  ...
  check(
    "No Area preview in the Reveal",
    (await fresh
      .locator('main [data-square][class*="shadow-area-preview"]')
      .count()) === 0,
  );

  const delays = await cells(fresh).evaluateAll((elements) =>
    elements.map((el) => el.style.animationDelay),
  );
  check(
    "Every Square is part of the wave",
    delays.every((delay) => delay.endsWith("ms")),
  );
  ...
  const replayed = await cells(fresh).evaluateAll(
    (elements) =>
      elements.filter((el) =>
        el.getAnimations().some((a) => a.animationName === "ffRevealIn"),
      ).length,
  );
  check(
    "Hover and pin do not replay the wave",
    replayed === 0,
    `${replayed} replaying`,
  );
  ...
  check(
    ...
    (await fresh.getByLabel("Your name").inputValue()) === "",
  );
}
